...
public class TableQueries {
	...
	public static boolean addBlackBoardMessage(Long tableKey,BlackBoardMessage bbMessage) {
		PersistenceManager pm = ServiceFactory.getPmfInstance().getPersistenceManager();
	...
	public static boolean addResource(String DocId, User user, Long tableKey) {
		...
		URL feedUri;
		try {
			feedUri = new URL("https://docs.google.com/feeds/documents/private/full/");
			DocumentListFeed feed = client.getFeed(feedUri,DocumentListFeed.class);
			DocumentListEntry docToAdd = null;
			// System.out.println("DocId = " + DocId);
			for (DocumentListEntry entry : feed.getEntries()) {
				// System.out.println("entry.getDocId() = " + entry.getDocId());
				if (entry.getDocId().equals(DocId)) docToAdd = entry;
			}
			if (docToAdd == null) return false;
			// Ho recuperato il doc da condividere, devo recuperare gli utenti
			// del gruppo con cui condividere il doc
			List<User> usersToShare = UserQueries.queryUsers(table.getMembers());
			URL url = new URL("https://docs.google.com/feeds/acl/private/full/"+ docToAdd.getResourceId());
			AclFeed aclFeed = client.getFeed(new URL(docToAdd.getAclFeedLink().getHref()), AclFeed.class);
			boolean skip;
			for (User u : usersToShare) {
				skip = false;
				for (AclEntry entry : aclFeed.getEntries())
					if (entry.getScope().getValue().equals(u.getEmail()))
						if (entry.getRole().getValue().equals("owner")|| entry.getRole().getValue().equals("writer"))
							skip = true;
				if (!skip) {
					AclRole role = new AclRole("writer");
					AclScope scope = new AclScope(AclScope.Type.USER,u.getEmail());
	...
	public static List<DriveFile> getTableDriveFiles(Table table) {
		...
		if (tableDriveFiles.size() == 0) return null;
		else return tableDriveFiles;
	}
