...
public class BookmarkWindow extends WindowPlus {
	...
	private void setTitle() {
		...
		title = new Html("<div align=\"center\"><b>\nThe " + objectName
				+ " <u>" + resource.getTitle()
				+ "</u> has been shared on this table\n\n</b></div> "
				+ "<div align=\"center\">\nUrl: " + resource.getUrl()
				+ "</div> ");
		title.setHeight(40);
	...
	private void setFrame() {
		...
		hpp.add(new Html("<iframe src='" + resource.getUrl()
				+ " width='300'; height='223'<p><a href='" + resource.getUrl()
				+ "'>" + resource.getTitle() + "</a></p></iframe>"));
		add(preview);
	}
	...
	private void createFlexTable() {
		...
		editLegendButton.setToolTip(new ToolTipConfig(
				"Edit the bookmark's legend"));
		...
		addAnnotation
				.setToolTip(new ToolTipConfig("Add annotation to bookmark"));
		...
		if (resource.getAnnotation().size() == 0)
			removeAnnotation.disable();
		removeAnnotation.setToolTip(new ToolTipConfig(
				"Remove annotation's bookmark"));
	...
	private void createGridComment() {
		...
		editItem.addSelectionListener(new SelectionListener<MenuEvent>() {
			...
			public void componentSelected(MenuEvent ce) {
				...
				bookmarkService.editComment(resource, key,
						new AsyncCallback<String>() {
							...
							public void onSuccess(String c) {
								...
								if (edit) {
									...
									contextMenu.setEnabled(commentStore
											.getCount() > 0);
								}
							}
						});
	...
	private void addComment() {
		if (inputArea.getValue() != null) {
			Comment c;
			if (radio.getValue())
				c = new Comment(inputArea.getValue(), TablePlus.getUser()
						.getEmail());
			else
				c = new Comment(inputArea.getValue(), TablePlus.getUser()
						.getEmail(), VisibilityType.PRIVATE);
			bookmarkService.addComment(resource.getKey(), c,
					new AsyncCallback<Boolean>() {
						@Override
						public void onFailure(Throwable caught) {
							GWT.log("Failed to add comment to bookmark: ",
									caught);
							Info.display("Error",
									"Failed to add comment to bookmark.");
						}
		...
		if (resource.getTag().size() > 0) {
			...
			listTag.addChangeHandler(new ChangeHandler() {
				...
				public void onChange(ChangeEvent event) {
					inputTag.setValue(listTag.getItemText(listTag
							.getSelectedIndex()));
				}
			});
			hp1.add(listTag);
		} else
			inputTag.setWidth(140);
	...
	public Widget getTag() {
		return new Html("<div style=\"padding-left:10px;\"><b>Tag:</b>"
				+ "<div style=\"padding-top:5px;\">" + resource.getTagString()
				+ "</div></div>");
	}
				...
				new AsyncCallback<Boolean>() {
					...
					public void onFailure(Throwable caught) {
						GWT.log("Unable to edit the bookmark's legend! ",
								caught);
						Info.display("Error",
								"Unable to edit the bookmark's legend.");
						unmask();
					}
					...
					public void onSuccess(Boolean result) {
						...
						Info.display("Succes",
								"Successfully edited bookmark's legend.");
		...
		bookmarkService.queryBookmark(resource.getKey(),
				new AsyncCallback<Bookmark>() {
					...
					public void onFailure(Throwable caught) {
						...
						Info.display("Error",
								"Unable to refresh the bookmarks.");
					}
	...
	public Widget getAnnotation() {
		return new Html("<div style=\"padding-left:10px;\"><b>Annotation:</b>"
				...
				+ resource.getAnnotationString() + "</div></div>");
	}
	...
	public void addAnnotation(final String annotation) {
		bookmarkService.addAnnotation(resource.getKey(), annotation,
				new AsyncCallback<Boolean>() {
					@Override
					public void onFailure(Throwable caught) {
	...
	public void removeAnnotation(final int annotation) {
		bookmarkService.removeAnnotation(resource.getKey(), annotation,
				new AsyncCallback<Boolean>() {
					@Override
					public void onFailure(Throwable caught) {
					...
					public void onSuccess(Boolean result) {
						...
						Info.display("Success",
								"Successfully removed annotation");
	...
	private void editComment(final Comment comment, final String key) {
		bookmarkService.addComment(resource.getKey(), comment,
				new AsyncCallback<Boolean>() {
					@Override
					public void onFailure(Throwable caught) {
						GWT.log("Failed to post comment: ", caught);
