...
public class TableObjectsWindow extends WindowPlus {
	...
	private List<TableObject> resource;
	...
	public TableObjectsWindow() {
		...
		openItem.addSelectionListener(new SelectionListener<MenuEvent>() {
			...
			public void componentSelected(MenuEvent ce) {
				final BaseModel selected = grid.getSelectionModel().getSelectedItem();
				String key = selected.get("key").toString();
				tableService.queryObject(key,new AsyncCallback<TableObject>() {
					@Override
					public void onFailure(Throwable caught) {
						GWT.log("Unable to load objetct for user: "+ 
								TablePlus.getDesktop().getActiveTable().getName(), caught);
						Info.display("Error","Unable to load objetct.");
						unmask();
					}
					@Override
					public void onSuccess(TableObject b) {
						if (b.getProvider().toString().equals("BOOKMARK"))
							TablePlus.getDesktop().showBookmarkWindow((Object)b);
						
						// TODO da implementare per finestra generica
					}
				});
			}
		});
		...
		addBookmarkButton = new Button("Add Bookmark", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				showInputPanel();
			}
		});
		addBookmarkButton.setToolTip(new ToolTipConfig("Add Bookmark"));
		addBookmarkButton.setIcon(IconHelper.createStyle("add"));
		
		
		
		addObjectButton = new Button("Add Object", new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				// TODO showInputPanel();
			}
		...
		addObjectButton.setToolTip(new ToolTipConfig("Add Object"));
		addObjectButton.setIcon(IconHelper.createStyle("add"));
	...
	private void commentView(final TableObject b) {
		...
		editItem.addSelectionListener(new SelectionListener<MenuEvent>() {
			...
			public void componentSelected(MenuEvent ce) {
				...
				tableService.editComment(b, key,new AsyncCallback<String>() {
					@Override
					public void onFailure(Throwable caught) {
	...
	private void showInputPanel(TableObject b, String c, String key) {
	...
	private FormPanel buildPanel(final TableObject b, String c, final String key) {
	...
	private void rebuildCommentView(TableObject b) {
	...
	private void editComment(final TableObject b, final Comment comment, final String key) {
		bookmarkService.addComment(b.getID(), comment,new AsyncCallback<Boolean>() {
			@Override
			public void onFailure(Throwable caught) {
	...
	private HorizontalPanel commentButton(final TableObject b) {
	...
	private void loadGridComments(final TableObject b) {
		commentStore.removeAll();
		bookmarkService.getComments(b.getID(), new AsyncCallback<List<Comment>>() {
			@Override
			public void onFailure(Throwable caught) {
				GWT.log("Unable to load comments for bookmark: "+ b.getName(), caught);
	...
	private void showFilterPanel() {
		...
		mainContainer.remove(grid);
		loadButton.disable();
		addBookmarkButton.disable();
		addObjectButton.disable();
		filterButton.disable();
		mainContainer.setLayout(centerLayout);
		mainContainer.add(inputPanel);
		mainContainer.layout();
	...
	private FormPanel buildTagPanel() {
		...
		saveButton.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				List<TableObject> tagFilter = new ArrayList<TableObject>();
				for (TableObject b : resource) {
	...
	private void addNewBookmark(final Bookmark o) {
		userService.addBookmark(TablePlus.getUser().getKey(),o,new AsyncCallback<Boolean>() {
			@Override
			public void onFailure(Throwable caught) {
	...
	private void fillGrid(List<TableObject> bookmarks) {
		allTags = new ArrayList<String>();
		for (final TableObject bookmark : bookmarks) {
	...
	private FormPanel newComment(final TableObject b) {
		final FormPanel panel = new FormPanel();
		FormData formData = new FormData("-20");
	...
	public void addComment(final TableObject b, Comment c) {
		bookmarkService.addComment(b.getID(), c,new AsyncCallback<Boolean>() {
			@Override
			public void onFailure(Throwable caught) {
				GWT.log("Failed to post comment: ", caught);
				Info.display("Error", "Failed to post comment.");
